...
class EventDetailsViewModel(
    ...
    fun refresh() {
        currentEventId?.let { loadEvent(it, force = true) }
    }
    ...
    private fun buildMainInfoState(event: Event): EventMainInfoUiState {
        val scheduleText = when {
            ...
            event.startDate.isNotBlank() && event.endDate.isNotBlank() -> "${event.startDate} - ${event.endDate}"
            event.startDate.isNotBlank() -> event.startDate
            event.endDate.isNotBlank() -> event.endDate
            else -> null
        }
    ...
    private fun buildActionsState(event: Event, isActionInProgress: Boolean): EventActionsUiState {
        val participantsLimitReached = event.participantLimit?.let { limit ->
            event.participantsCount >= limit
        } ?: false

        val primaryButton = when {
            event.isFinished && event.isUserParticipant -> EventPrimaryButtonState(
                text = "Вы участвовали",
                enabled = false,
                action = EventPrimaryAction.NONE
            )

            event.isFinished && !event.isUserParticipant -> null

            event.isUserParticipant -> EventPrimaryButtonState(
                text = "Не смогу пойти",
    ...
    private fun buildAdditionalInfoState(event: Event): EventAdditionalInfoUiState {
        return EventAdditionalInfoUiState(
            ...
            howToGet = event.howToGet.takeIf { it.isNotBlank() },
